...
ESX.RegisterServerCallback('evan-robberywarung:lockpick', function(source, cb, item, count)
    local src = source
...
AddEventHandler('esx_holdup:robberyStarted', function(currentStore)
	...
	if Stores[currentStore] then
		...
		if (os.time() - store.lastRobbed) < Config.SebelumRobBaru and store.lastRobbed ~= 0 then
			TriggerClientEvent('esx:showNotification', _source, _U('recently_robbed', Config.SebelumRobBaru - (os.time() - store.lastRobbed)))
			return
		end
		...
		if not rob then
			if cops >= Config.MinimalPolisi then
				rob = true
				...
				TriggerClientEvent('esx:showNotification', _source, _U('min_police', Config.MinimalPolisi))
			end
		else
